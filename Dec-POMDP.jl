...
function verify_satellite_controller(joint_controller::JointController, prob::MultiPacketSatelliteNetworkPOMDP, num_episodes=1000, max_steps=50)
    ...
    # Episode trace collection - one trace per episode
    all_episode_traces = []
    ...
    for episode in 1:num_episodes
        ...
        # Create a new episode trace for each episode
        episode_trace = Vector{NamedTuple{(:step, :state, :actions, :packet_counts, :packets_transmitted_this_step),
        Tuple{Int, String, Tuple, Vector{Int}, Int}}}()
        ...
        while step_count < max_steps && state != "all-transmitted"
            ...
            # Add action to history - before action execution to capture planned actions
            push!(episode_trace, (
                step=step_count,
                state=state,
                actions=joint_action,
                packet_counts=copy(packet_counts),
                packets_transmitted_this_step=0  # Will be updated after execution
            ))
            ...
            # Track packets transmitted in this step
            ...
            # Update the packets transmitted for this step in the trace
            episode_packets_transmitted += packets_transmitted_this_step
            if !isempty(episode_trace)
                episode_trace[end] = merge(episode_trace[end], 
            ...
            if state == "all-transmitted"
                ...
                observations = [empty_obs for _ in 1:prob.num_satellites]  # FIXED: removed asterisk
            else
        ...
        # Store the trace for this episode
        push!(all_episode_traces, episode_trace)
    end
    ...
    return avg_reward, completion_rate, all_episode_traces 
end

# Helper function to parse state strings
function parse_state(state::String)
    if state == "all-transmitted"
        return []
    else
        return [parse(Int, s) for s in split(state, "-")]
    end
end

# Verification function to simulate and analyze the controller
function verify_satellite_controller_original(joint_controller::JointController, prob::MultiPacketSatelliteNetworkPOMDP, num_episodes=1000, max_steps=50)
    total_reward = 0.0
    rewards_per_episode = []
    packets_transmitted = 0
    episodes_completed = 0
    ...
    for episode in 1:num_episodes
        ...
        while step_count < max_steps && state != "all-transmitted"
            ...
            # Update the packets transmitted for this step in the trace
            episode_packets_transmitted += step_packets_transmitted
            if !isempty(episode_trace)
                episode_trace[end] = merge(episode_trace[end], 
                    (packets_transmitted_this_step=step_packets_transmitted,))
            end
        ...
        # Only keep trace for the first episode if running multiple episodes
        if episode > 1
            empty!(episode_trace)
        end
    end
    ...
    return avg_reward, completion_rate, episode_trace 
end
...
function plot_satellite_trace(all_episode_traces, episode_index, num_sats::Int)
    # Get the history for the specific episode
    if isempty(all_episode_traces) || episode_index > length(all_episode_traces)
        error("Invalid episode index or empty traces")
    end
    
    history = all_episode_traces[episode_index]
    timesteps = length(history)
    
    # Extract data from history
    states = [h.state for h in history]
    actions = [h.actions for h in history]
    packet_counts = [h.packet_counts for h in history]
    transmissions = [h.packets_transmitted_this_step for h in history]
    
    # Create plots using Plots.jl
    p1 = plot(title="Packet Distribution Over Time (Episode $episode_index)", 
              xlabel="Timestep", ylabel="Packets",
              legend=:bottomright, size=(800, 400))
    
    # Plot packet counts for each satellite
    sat_data = []
    for s in 1:num_sats
        counts = [pc[s] for pc in packet_counts]
        plot!(p1, 1:timesteps, counts, label="Sat $s", lw=2, marker=:circle, markersize=4)
        push!(sat_data, counts)
    end
    
    # Plot transmissions (both per-step and cumulative)
    plot!(p1, 1:timesteps, transmissions, label="TX per step", 
          lw=2, lc=:red, marker=:diamond, markersize=4)
    
    cum_tx = cumsum(transmissions)
    plot!(p1, 1:timesteps, cum_tx, label="Cumulative TX", 
          lw=2, lc=:black, ls=:dash)
    
    # Create action visualization
    p2 = plot(title="Satellite Actions",
              xlabel="Timestep", ylabel="Satellite",
              yticks=(1:num_sats, ["Sat $i" for i in 1:num_sats]),
              legend=false, size=(800, 200))
    
    # Use rectangular heatmap for action backgrounds
    action_colors = Dict(
        "transmit" => :lightgreen,
        "pass-left" => :lightblue,
        "pass-right" => :cyan,
    ...
    # First create a blank background
    action_matrix = zeros(Int, num_sats, timesteps)
    
    # Add annotations for each action
    for t in 1:timesteps
        for s in 1:num_sats
            # Access the action, handling different tuple types
            action = if actions[t] isa Tuple
                actions[t][s]
            elseif actions[t] isa AbstractVector
                actions[t][s]
            else
                string(actions[t])  # Fallback
            end
            
            # Determine color based on action type
            bg_color = :lightgray  # Default
            for (key, val) in action_colors
                if startswith(action, key)
                    bg_color = val
                    break
                end
            end
            
            # Add colored rectangle in background
            scatter!(p2, [t], [s], marker=:square, markersize=18, 
                    color=bg_color, alpha=0.7, markerstrokewidth=0)
            
            # Create shorter action label
            short_action = if startswith(action, "transmit")
                parts = split(action, "-")
                if length(parts) > 1
                    "TX-" * parts[2]
                else
            ...
            elseif startswith(action, "pass")
                parts = split(action, "-")
                if length(parts) > 2
                    direction = parts[2][1]  # First letter of direction
                    "P$direction-" * parts[3]
                else
                    "Pass"
                end
            ...
            # Add text label
            annotate!(p2, t, s, text(short_action, 8, :black))
        end
    end
    
    # Combine plots
    plot(p1, p2, layout=grid(2, 1, heights=[0.7, 0.3]), size=(900, 700))
end

# Function to plot statistics across all episodes
function plot_episode_statistics(all_episode_traces, num_sats::Int)
    num_episodes = length(all_episode_traces)
    
    # Extract episode-level statistics
    episode_steps = [length(trace) for trace in all_episode_traces]
    
    # Calculate the total packets transmitted per episode
    episode_transmissions = zeros(Int, num_episodes)
    for (i, trace) in enumerate(all_episode_traces)
        total_tx = 0
        for step in trace
            total_tx += step.packets_transmitted_this_step
        end
        episode_transmissions[i] = total_tx
    end
    
    # Plot episode completion steps
    p1 = plot(title="Episode Length", 
              xlabel="Episode", ylabel="Steps to Completion",
              legend=false, size=(800, 300))
    plot!(p1, 1:num_episodes, episode_steps, lw=2, marker=:circle, 
          color=:blue, markersize=3, alpha=0.7)
    hline!(p1, [mean(episode_steps)], lw=2, ls=:dash, color=:red, 
           label="Mean: $(round(mean(episode_steps), digits=2))")
    
    # Plot packets transmitted per episode
    p2 = plot(title="Packets Transmitted", 
              xlabel="Episode", ylabel="Total Packets",
              legend=false, size=(800, 300))
    plot!(p2, 1:num_episodes, episode_transmissions, lw=2, marker=:circle, 
          color=:green, markersize=3, alpha=0.7)
    hline!(p2, [mean(episode_transmissions)], lw=2, ls=:dash, color=:red, 
           label="Mean: $(round(mean(episode_transmissions), digits=2))")
    
    # Combine plots
    plot(p1, p2, layout=grid(2, 1), size=(900, 600))
end

# Function to compare multiple specific episodes
function compare_episodes(all_episode_traces, episode_indices, num_sats::Int)
    # Number of episodes to compare
    num_to_compare = length(episode_indices)
    if num_to_compare == 0
        error("No episode indices provided")
    end
    
    # Create plot for packet distribution
    p = plot(title="Packet Distribution Comparison", 
             xlabel="Timestep", ylabel="Packets",
    ...
    # Plot data for each selected episode
    for (idx, episode_idx) in enumerate(episode_indices)
        if episode_idx > length(all_episode_traces)
            @warn "Episode $episode_idx does not exist, skipping"
            continue
        end
        
        trace = all_episode_traces[episode_idx]
        timesteps = length(trace)
        
        # Get cumulative transmissions for this episode
        transmissions = [step.packets_transmitted_this_step for step in trace]
        cum_tx = cumsum(transmissions)
        
        # Plot with cycling styles
        ls = line_styles[mod1(idx, length(line_styles))]
        color = colors[mod1(idx, length(colors))]
        
        plot!(p, 1:timesteps, cum_tx, 
              label="Episode $episode_idx", 
              lw=2, 
              ls=ls,
              color=color)
    end
...
avg_reward, completion_rate, all_episode_traces = verify_satellite_controller(final_controller, sat_network)
...
num_episodes = length(all_episode_traces)
println("Collected traces for $num_episodes episodes")

# Plot the first episode as a detailed example
if !isempty(all_episode_traces)
    p1 = plot_satellite_trace(all_episode_traces, 1, sat_network.num_satellites) 
    display(p1)
    savefig(p1, "satellite_simulation_episode1.png")
    
    # Plot overall statistics across all episodes
    p2 = plot_episode_statistics(all_episode_traces, sat_network.num_satellites)
    display(p2)
    savefig(p2, "satellite_simulation_statistics.png")
    
    # Compare a few episodes (e.g., first, middle, last)
    if num_episodes >= 3
        episode_indices = [1, div(num_episodes, 2), num_episodes]
        p3 = compare_episodes(all_episode_traces, episode_indices, sat_network.num_satellites)
        display(p3)
        savefig(p3, "satellite_simulation_comparison.png")
    end
else
    println("No episode traces were collected.")
end